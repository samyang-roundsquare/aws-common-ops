...
echo "=========================================="
echo " AWS EC2 Ubuntu Initialization Script"
echo "=========================================="

# 1. Root User Check
if [ "$(id -u)" -ne 0 ]; then
    # echo "Current user is not root. Switching to root..."
    sudo -i
    if [ $? -ne 0 ]; then
        # echo "sudo -i failed. Re-running script with sudo..."
        exec sudo bash "$0" "$@"
        exit
    fi
fi
...
if command -v apt-get &>/dev/null; then
    ...
    apt-get install -y git cron jq curl awscli
else
    echo "Error: This script supports 'apt-get' package manager (Ubuntu/Debian)."
    exit 1
fi
...
# Install Docker Compose Plugin : compose build requires buildx 0.17 or later -> 2.39.1
...
if [ -f ~/.aws/credentials ]; then
    ...
    # Configure AWS CLI for ubuntu user as well (optional but good for convenience)
